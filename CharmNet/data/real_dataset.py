...
class RealDataset(BaseDataset):
    ...
    def __init__(self, opt):
        ...
        if opt.isTrain==True:
            print('loading training file in real domain: ')
            self.trainfile = opt.dataset_root+opt.real_base_list
            ...
            print('loading test file in real domain: ')
            self.keep_background_prob = -1
            self.trainfile = opt.dataset_root+'IHD_human_test.txt'
            with open(self.trainfile,'r') as f:
                    for line in f.readlines():
                        self.image_paths.append(os.path.join(opt.dataset_root,line.rstrip()))
